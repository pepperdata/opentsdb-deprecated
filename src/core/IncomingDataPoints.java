  ...
  */
  ...
  private Deferred<Object> addPointInternal(final long timestamp, final byte[] value,
    ...
    if (ms_timestamp) {
      ...
      incoming_base_time = (timestamp - (timestamp % Const.MAX_TIMESPAN_SECS));
    }
    
    if (incoming_base_time - base_time >= Const.MAX_TIMESPAN_SECS) {
      // Need to start a new row as we've exceeded Const.MAX_TIMESPAN_SECS.
