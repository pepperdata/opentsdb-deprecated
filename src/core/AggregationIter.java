 ...
 * <p>
 *
 * Interfaces of {@link Aggregator.Longs}:
 * {@link SeekableView}:
 *   {@link AggregationIter} should be used as an iterator like:
 * <pre>
 *   iter = AggregationIter(...);
 *   iter.seek(timestamp);
 *   while (iter.hasNext()) {
 *     DataPoint dp = iter.next();
 *     dp.longValue(); // or dp.doubleValue()
 *   }
 * </pre>
 *   iter.next moves to the next time series data point by moving
 *   {@link #current} and returns 'this' AggregationIter object.
 *
 * {@link DataPoint}:
 *   dp.longValue aggregates values across multiple spans using the given
 *   aggregator. It calls aggregtor.runLong, which takes AggregationIter as
 *   an Aggregator.Longs.
 *
 * {@link Aggregator.Longs}:
 *   Iterates long values of the current time stamp across spans.
 *   {@link #hasNextValue}: True if next span is available. False at the last
 *       span.
...
final class AggregationIter implements SeekableView, DataPoint,
  ...
  public double nextDoubleValue() {
    if (hasNextValue(true)) {
      ...
      if (rate) {
        // No LERP for the rate. Just uses the rate of any future timestamp.
        if (x < x0) {
          return y0;
        }
        return y1;
